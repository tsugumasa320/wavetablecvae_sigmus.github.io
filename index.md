...
本研究は、データに基づいた意味的なウェーブテーブル制御の実現による音色探索の直感性向上を目的する。

## 序論

音楽制作やパフォーマンスにおいて、シンセサイザーは重要な役割を果たしている。
しかし、望む音色を生成するためには、知識と経験が必要である。
多くの音響合成方式が存在するが、ここではデジタル音響合成の最小単位である「ウェーブテーブル合成」に注目する。
1周期分の波形を対象とする事で、ウェーブテーブル合成及び加算/減算合成,AM/FMなどの他合成方式にも適用できるものとする。
...
### ②. 教師ラベルの算出
  ...
  - Kreković[^3]は、bright, warm, richの３種類のアトリビュートラベルをそれぞれ、スペクトル重心、スペクトル密度、奇数時倍音のエネルギー比から計算している
...
### ③. モデル構成
  ...
  - 学習率は0.001で、Adam optimizerを採用.バッチサイズは32、エポック数は30000回
...
本研究では、ウェーブテーブル合成における意味的な音色制御を可能にする手法を提案した。
特に、ラベルを変化させることで、ウェーブテーブルの音色を操作することができることを示した。
ただし、再構成精度やラベルによる操作の滑らかさに課題が残っていることも確認された。

また、提案手法では、音響特徴に基づいて算出した3つのラベルを用いた条件付け生成を行った。
しかし、ラベルの種類や数を増やすことで、より細かい音色の制御が可能となると考えられる。
今後は、より多様なラベルを用いた音色制御の実現を目指すことが重要である。

データセットとして、Adventure Kid Research & Technologyが提供するモノラルのウェーブテーブル4158件を使用した。
データセットの増加によって、より多様な音色の生成が可能となると考えられる。今後は、より多様なウェーブテーブルを含むデータセットの構築が求められる。
また、生成された音色の評価指標として、人間の主観的な評価や音響特徴を用いた客観的な評価などが考えられる。
今後は、生成された音色の評価についても検討を進める必要がある。
