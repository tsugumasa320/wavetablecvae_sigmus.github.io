...
条件付けには、音響特徴に基づいて算出した明るさ(bright), 温かさ(warm), リッチ(rich)という3つのアトリビュートラベルを用いる。
...
実験の結果、提案手法は、テストデータに対して高い再構成の忠実度を示し、アトリビュートラベルを変化させることでウェーブテーブルの音色を操作できることを定性的・定量的に示す。
本研究は、知覚に基づいたデータ・ドリブンな音色操作を目指すものである。
...
![スクリーンショット 2023-02-15 16 47 52](https://user-images.githubusercontent.com/35299183/218964891-25e1b1f9-c563-4d08-ab14-f08ad83f0eb8.png)

### 背景

  - シンセサイザーは様々なメディアや音楽制作で重要な役割を果たしている
  - しかし、任意の音色を生成するには知識と経験が必要という課題がある
  - デジタル音響合成において基本的な音生成方式である、ウェーブテーブル合成に注目.
    - 深層生成モデルで上記課題の改善を狙う
