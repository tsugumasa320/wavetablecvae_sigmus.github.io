...
- どこに独創性(オリジナリティ)があるのか？
  ...
  - 先行研究と比較して高解像度なデータセットとそれに対応するモデル
 ...
 - 他の先行研究、類似研究との関係はどうなっているのか？
   - Krekovićは、CVAEモデルで音色の生成を行ったがウェーブテーブルのサイズを3つのパラメータに圧縮し、
   - アップサンプリングを行うというモデル構成を採用しているため、生成できる音色が限られていると推測する
   - Hyrkasらは、VAEで音色の操作を行いましたが、音色の特徴を表現するパラメータがなかったため、操作が不自由
 - 先行研究から変わった点はどこか？
   - セマンティックラベルで操作でき多様なウェーブテーブルをエフェクターの様に操作できる点
   - リアルタイム性にこだわって、推論時に周波数領域に変換せずに実施できるように取り組んでいる点
   - より実用に近い解像度の高いデータセットを構築し使用している点
...
### 実験結果の客観性や論理性（以降は結果が出てから追記)
 - 他の条件では実験を行ったのか？
    ...
    - バッチサイズや学習率などハイパラを詰めるところはまだできていない
  
 - その研究結果から主張までに論理の飛躍が起きていないか？
    - 大丈夫かな？
 - 実験結果から伝えたいメッセージはなにか？
    - 生成と条件付けが上手くいって、音色の操作がある程度できている点
 - 別の実験結果が必要ではないか？
    - 要検討 
 - 複数の実験結果を適切に比較しているか？
    - 要検討
...
### 湯谷追加
 ...
 - なぜウェーブテーブル合成？
    ...
    - 音楽用途のAIソフトウェアがあまり使用されておらず、既存のシンセの課題を深層学習で解決するというアプローチを取った
    - 基本的な方式であるため、他の合成方式へも応用ができるのではないかと考えているため
 - 他の合成方式への応用とは？
    - 先ず、AM/FM/加算/減算はできる。LFOやFMのモジュレーターとして同じ様に音響特長量でラベル付して操作するということは可能だと考える
    - ↑ここはディスカッションさせて欲しい点
 - なぜLReLU？
    - 先行研究を(RAVE)を参考にモデルを作成したのでこれを使用している
    - BNと区分線型関数の組み合わせの実績があったため（ここも要検討ではある）
 - なぜResBlock?
    - 層は深く無いので、必然性はないかも,ここは要検討項目
 - なぜVAE？GANは？
    - GANでも同じようなことはできるかもしれないが、そこは試していない
    - GANを使用するメリットは例えば、解像度を高くできるなどある
    - しかし、600次元程度なのでわざわざ学習の難しいGANを採用する理由がない
